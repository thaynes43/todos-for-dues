...
9. **PAT for release-please** (the actual fix for the `GITHUB_TOKEN`-trap). Confirmed 2026-05-18 post-v0.7.0 release: `release: types: [published]` events created by `GITHUB_TOKEN` are ALSO suppressed (same security policy as tag-push events). The `release` event swap from PLAN-013 Track A did NOT close the trap on its own — it just changed the workaround from "manual tag re-push" to "manual release re-publish". A separate `fix(ci):` PR landed post-v0.7.0 to restore the `push: tags` trigger as a hybrid fallback so BOTH workarounds work; PAT remains the proper long-term fix. **Real fix:** mint a fine-grained PAT (repo: contents:write + actions:read+write); add as repo secret `RELEASE_PLEASE_PAT`; update `.github/workflows/release-please.yml` to consume it. Then release-please's actions originate from a real user identity and fire downstream workflows automatically. User authorizes the secret; coordinator authors the workflow update.
