...
- **Pre-beta validation plan (post-PLAN-012)** — VALIDATION-009 deferred 3 user-driven gates (walking-skeleton live click-through, treasurer email lands, SSO bootstrap-admin live click-through) per §7.1 because the Admin UI surface ships incrementally across PLAN-010/011/012; running them earlier means re-running after every feature plan. Treat as a SEPARATE plan (likely `PLAN-014` or appended to `docs/releases/001-mvp.md` §5 rollout) that runs after PLAN-012 completes. **Scope:** the 3 deferred gates + any user-driven smoke that lands during PLAN-010/011/012. Treat as the last gate before bringing in beta testers — chapter-data hygiene matters once real users arrive. Discuss during the post-deploy review with the user; decide plan vs. release-doc-appendix.
